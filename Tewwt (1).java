...
public class Tewwt
{
    ...
    public static void main(String[] args) throws OAuthMessageSignerException, OAuthExpectationFailedException, OAuthCommunicationException, IOException, ParseException, JSONException, SQLException 
    {
        ...
        String sqlquery = "insert into tweetsInfo (SR_NUMBER, TWEET_ID, TWEET_TIME, USER_NAME, USER_ID,"
                           + " NUMBER_OF_RETWEETS, NUMBER_OF_FAVORITE, TWEET_TEXT, POSITIVE_STRENGTH, NEGATIVE_STRENGTH, INCLUDED_URL) values (?,?,?,?,?,?,?,?,?,?,?)";
        pst = conn.prepareStatement(sqlquery);
        
        int flag = 0, k = 1, positiveStrength = 0, negativeStrength = 0;
        //max_id = "483205821564981248";
       //loop to iterate through timeline 
        for(int  j = 0; j < 1000; j++)
        {
            ...
            else
            {
                ...
                url = "https://api.twitter.com/1.1/search/tweets.json?max_id="+max_id+"&q="+keyword+"&count=50";
            }
            //retrive data 
